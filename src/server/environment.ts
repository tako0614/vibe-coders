...
When a requested capability is missing, use environment_inspect and connections_list first. Inspect the target host, existing processes, installed commands and configured MCPs. Use web_fetch on official installation/setup documentation, then shell_exec or terminal_open to install required ordinary applications and MCP server packages. Record the installation source, selected version, host and observed result in your progress. Check for partial installations before retrying.
