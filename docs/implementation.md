...
| 機能           | 実装内容                                                                                                                                                                   |
| -------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| チャット       | 会話・画像・テキスト・PDF添付、ストリーム、追加入力、親の一時停止と再開、全停止、長い会話の自動要約と元の履歴保持、下書きのブラウザ保存                                                                                            |
...
- 型検査、Bunの74テスト、Reactビルド、Honoによる認証付き画面配信（0.2.0）。
...
- 0.2.0の常用LAN版で親の接続先をCodexへ設定し、端末の既存認証と `gpt-6-astra` による実テキスト応答を確認。実Chromeでも返答と接続済み表示を確認。モデル一覧のApp Server起動失敗は残っています。
