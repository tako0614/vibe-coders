...
Bun 1.3.14以上を先にインストールしてください。0.3.0は配布tarballから導入できます。npm公開済み版は0.1.8です。
...
MCPは「設定・接続 → MCP → 接続を追加」でコマンド・HTTP接続を手動登録し、一覧から編集・認証できます。AIに任せる場合はチャットから「Chromeを導入してMCPで接続して」と依頼できます。親がOS・既存環境を確認し、通常のChromeと必要なMCPを導入・登録・接続します。親モデルと対象ホストへの操作経路が必要です。Chromeのリモートデバッグ許可など、本人操作が必要な箇所は入力カードへ回します。
...
チャットの「シェル」「画面」から、同じ会話の実行環境を横に開けます。AIがシェルや画面を使い始めたときも自動で開きます。画面を閉じてもプロセスは継続し、AIの一時停止も起動済みシェルを終了しません。操作の引数・結果は会話中の履歴を展開すると確認できます。


チャット横の `＋` から複数の独立した仮想デスクトップを作成できます。画面ごとにシェル・Chromeプロファイル・操作権・MCPの対象を管理します。Codex/OpenRouterの対応モデルでは、チャットのモデル名の横からeffortを選択できます。
