...
2026-09-16。**0.2.1はチャット内のモデル選択とUIを改善しています。npm公開は認証待ち（401）で、公開済み版は0.1.8です。** 0.2.0ではデッキ・複数端末と共通shellを追加し、専用native子実行を廃止しました。検索・直接入力できるモデルpickerと既存Codex認証の再利用にも対応しています。[0.2.1の反映・検証記録](debugging-0.2.1.md)・[shellの仕様と移行](shell-workspace.md)を参照してください。以下のnpmインストール例は公開済み版です。
...
```sh
npm install -g /path/to/vibe-coders-0.2.1.tgz
vibe-coders setup
