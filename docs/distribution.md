...
「設定・接続 → MCP → npmパッケージから導入」でパッケージ名を指定すると、正確なバージョンへ固定して導入し、登録・接続・ツール取得を行います。親AIの接続は不要です。パッケージは設定ディレクトリ内の `mcp-packages/` に保存し、導入結果・失敗した工程を実行ログに残します。登録後の認証待ち・接続失敗は、専用入力と「接続」から再開できます。コマンドやHTTPサービスは「接続を追加」で登録できます。接続一覧の「編集」で設定を変更して再接続します。AIへの依頼フォームは設けず、AIに任せる場合は通常のチャットで依頼します。
...
「AI接続 → Codex」で端末の認証を確認し、「Codexを使う」で接続先を保存します。モデルはチャット入力欄のpickerから検索・選択するか、モデルIDを直接入力して確定します。保存済み認証がなければコード・ブラウザ認証を案内します。モデル一覧は端末のCodexのカタログ取得コマンドを使用し、会話DBを開きません。未対応の旧CLIのみApp Server経由で取得します。
...
OpenRouter・OpenAI互換APIもチャット内のpickerを使用します。設定画面でURLとAPIキーを保存したあと、チャットでモデル一覧の検索・選択・直接入力ができます。接続設定の保存時にモデル名は不要です。同じURLでモデルだけを変更すると保存済みキーを引き継ぎ、別URLへそのキーを送信しません。
...
「設定・接続 → AI接続」でCodex・OpenRouter・OpenAIを選択できます。別の互換APIは「接続先を追加」からURLを登録します。APIキーは専用欄へ入力して「保存して接続する」を押してください。保存済みならキーを入力し直す必要はありません。差し替えるときだけ入力し、削除するときは「保存済みAPIキーを削除」を使います。

チャットのモデル名を押すと、メニュー上部から接続先を切り替えられます。前回のモデル・effortも復元します。接続先は正規化したURL全体で区別し、別のホスト・パスへ保存済みキーを引き継ぎません。Codexは既存の端末認証を参照します。旧バージョンの使用中キーは最初の切り替え時に保存先へ引き継ぎます。

APIキーはconfig.json・会話・ブラウザ保存領域には書かず、既存のAES-256-GCM vaultで暗号化します。設定ファイルに保存するのは接続先・モデル・effortなどの非秘密情報です。実行中は接続先変更とキー削除を拒否します。
