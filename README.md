...
npm公開済み版は `npm install -g vibe-coders@0.1.8`、このソースの0.3.0は配布tarballから導入できます（npm公開は認証待ち）。起動手順は [配布・運用](docs/distribution.md) を参照してください。以下はソースからの起動手順です。
...
- MCPを手動で追加・編集・接続。チャットで頼む場合は親が設定ツールで導入・接続する
...
GUIのX11直接操作には `xdotool` とImageMagickが必要です。VNC方式なら別OSの画面にも接続できます。VNC接続先はバックエンドのloopbackで、別ホストはSSH転送等を使います。「手動操作」にすると管理下のAIアクセスを止め、「安全な画面でAIに返す」で返却します。ブラウザMCPが同じ画面を扱う場合は `targetId: "desktop:<id>"`（最初の画面は `desktop:default`） を設定してください。対応するVNC認証とOS別の準備は [配布・運用](docs/distribution.md) を参照してください。

「設定・接続 → MCP → 接続を追加」でコマンドまたはHTTP URLを登録し、一覧から編集・接続・認証できます。AI接続は不要です。AIに任せる場合はチャットから、たとえば「Chromeを導入してMCPで接続し、ページを操作できるようにして」と依頼できます。Chrome専用機能は持たず、親が通常のアプリとMCPを導入・登録します。既存のChromeや接続を優先し、必要な本人操作は入力カードへ回します。実行先ホストへの操作経路と、親モデルの接続設定が必要です。
...
0.2.2ではロゴ・回答アバター・常設の機能一覧を外し、会話履歴とチャットを中心に作り直しました。実行中はチャットと同じ画面で継続シェル・共有デスクトップを開き、手動操作へ引き継げます。コマンドの引数・結果は折りたたんだ実行履歴にまとめています。MCP設定は手動の追加・編集フォームを基本とし、AIへの設定依頼は通常のチャットから受けます。[検証記録](docs/debugging-0.2.2.md)。


0.3.0ではチャット横にデスクトップのタブを常設し、`＋`で独立した画面を追加できます。画面ごとにChromeプロファイル・シェルの接続先・操作権・MCPの対象を分けます。モデル名の横には、CodexとOpenRouterのモデルが対応するeffortを表示します。[仕様と検証](docs/debugging-0.3.0.md)。
